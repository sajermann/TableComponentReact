...
import { ForwardedRef, createRef } from 'react';
import { TAnimal, TBrawler, TChat, TPerson, TVehicle } from '~/types';
...
function chat(quantity: number) {
  const makeDataLevel = (): TChat[] => {
    const idOne = uuidv4();
    const userNameOne = faker.person.firstName();
    const userAvatarOne = faker.image.avatar();

    const idTwo = uuidv4();
    const userNameTwo = faker.person.firstName();
    const userAvatarTwo = faker.image.avatar();
